...
def create_distribution(d_list):
    #Converting the list of components into a dictionary
    modules = {}
    for b in d_list:
        for key in b:
            modules[key]=b[key]
    modules['aws.greengrass.Cli'] = {'componentVersion': '2.3.0'}
    #d['aws.greengrass.SecureTunneling'] = {'componentVersion': '1.0.3'}
    modules['aws.greengrass.Nucleus'] = {'componentVersion': '2.3.0'}

    #not sure if this is still required
    #iot = boto3.client('iot', region_name=os.environ['region_id'])
    ...
    response = gg_client.create_deployment(
            targetArn=gname_prefix+gname,
            deploymentName=gname,
            components=modules,            
            deploymentPolicies={
...
gname_prefix = "arn:aws:iot:"+os.environ['region_id']+":"+os.environ['account_id']+":thinggroup/"
gname='ri-drone-mock-group'
...
session = boto3.Session(
    region_name=os.environ['region_id'],
    aws_access_key_id='',
...
create_distribution(componentList)